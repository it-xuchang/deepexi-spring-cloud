...
import com.deepexi.common.annotation.Limit;
import com.deepexi.common.authentication.JWTToken;
import com.deepexi.common.authentication.JWTUtil;
import com.deepexi.common.exception.SystemException;
import com.deepexi.common.properties.FebsProperties;
import com.deepexi.common.util.*;
...
import com.deepexi.domain.vo.ActiveUser;
import com.deepexi.domain.vo.CommonConstant;
import com.deepexi.domain.vo.CommonResponse;
import com.deepexi.manager.UserManager;
...
public class LoginController {
    ...
    public CommonResponse login(
        ...
        return new CommonResponse().message("认证成功").data(userInfo);
    }
    ...
    @GetMapping("online")
    public CommonResponse userOnline(String username) throws Exception {
        String now = DateUtil.formatFullTime(LocalDateTime.now());
        Set<String> userOnlineStringSet = redisService.zrangeByScore(CommonConstant.ACTIVE_USERS_ZSET_PREFIX, now, "+inf");
        ...
        return new CommonResponse().data(activeUsers);
    }
    ...
    public void kickout(@NotBlank(message = "{required}") @PathVariable String id) throws Exception {
        ...
        if (kickoutUser != null && StringUtils.isNotBlank(kickoutUserString)) {
            // 删除 zset中的记录
            redisService.zrem(CommonConstant.ACTIVE_USERS_ZSET_PREFIX, kickoutUserString);
            // 删除对应的 token缓存
            redisService.del(CommonConstant.TOKEN_CACHE_PREFIX + kickoutUser.getToken() + "." + kickoutUser.getIp());
        }
    }
    ...
    private String saveTokenToRedis(User user, JWTToken token, HttpServletRequest request) throws Exception {
        ...
        this.redisService.zadd(CommonConstant.ACTIVE_USERS_ZSET_PREFIX, Double.valueOf(token.getExipreAt()), mapper.writeValueAsString(activeUser));
        // redis 中存储这个加密 token，key = 前缀 + 加密 token + .ip
        this.redisService.set(CommonConstant.TOKEN_CACHE_PREFIX + token.getToken() + StringPool.DOT + ip, token.getToken(), properties.getShiro().getJwtTimeOut() * 1000);
